import { CreditCard, Delete, ResetTvRounded } from "@mui/icons-material";
import {
  Button,
  Divider,
  Grid,
  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button
          variant="text"
          onClick={(e)=>{handleNewAddress({...newAddress, isAddingNewAddress:false, value:''})}}
        >
          Cancel
        </Button>
      </Stack>
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if(validateRequest(items,addresses)){
      ...
      if(response.data.success){
        ...
        history.push('/thanks')
        enqueueSnackbar('Order places succesfully',{variant:'success'})
        console.log(`this is wallet balance${balance}`)
        return true;
      }
      ...
      if(err.response){
        ...
        enqueueSnackbar('Something went wrong, please check your backend is running')
      }
